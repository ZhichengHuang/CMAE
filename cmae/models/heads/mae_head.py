import torch
from ..losses import LabelSmoothLoss, CrossEntropyLoss
...
class MAEFinetuneHead(BaseModule):
    ...
    def loss(self, outputs, labels):
        ...
        losses['acc'] = accuracy(outputs[0], labels)

        return losses

@MODELS.register_module()
class MAEFinetuneSMHead(BaseModule):
    """Fine-tuning head for MAE.
    ...
    def __init__(self, embed_dim, num_classes=1000, label_smooth_val=0.1):
        super().__init__()
        self.head = nn.Linear(embed_dim, num_classes)
        self.criterion = CrossEntropyLoss(use_soft=True)
